        ...
        while True:
            ...
            if not self.has_connection:
                ...
                try:
                    ...
                    logging.debug("CLIENT: failed to connect", exc)
                    self.has_connection = False

            if self.quit_event.is_set():
                logging.debug("CLIENT: quitting")
                break

            if self.has_connection:
                try:
                    logging.debug("Waiting for response from server")
                    response = self.soc.recv(1024)
                    logging.debug("CLIENT: response received", response)
                    if self.quit_event.is_set():
                        break
                    ...
                    self.has_connection = False
                    logging.debug("CLIENT: Connection lost", exc)
                if self.quit_event.is_set():
                    break
            if self.quit_event.is_set():
                break
...
class SimpleEnqueueSocketClient(SocketClient):
    ...
    def message(self, msg):
        ...
        """
        logging.debug("CLIENT: MESSAGE RECEIVED", msg)
        self.queue.put(msg)
    ...
    def handle_response(self, response):
        ...
        :return: None
        """
        logging.debug("CLIENT: handle response")
    ...
    def retry_messages(self):
        ...
        for message in failed_messages:
            try:
                ...
                logging.debug(f"CLIENT: error in retry_message: {err}")
                pass
    ...
    def callback(self, message):
        ...
        try:
            ...
            self.disconnect()
            self.initialize_socket()
            logging.debug("CLIENT: lost connection to server")
        except Exception as err:
            logging.debug(f"CLIENT: error in callback: {err}")
    ...
    def handle_response_default(self, msg):
        ...
        :return: None
        """
        logging.debug("CLIENT: Pass handle_response to kwargs to override this method")
    ...
    def response_worker(self):
        ...
        """
        while True:
            logging.debug("Client waiting for message...")
