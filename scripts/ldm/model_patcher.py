...
class ModelPatcher:
    ...
    def model_size(self):
        ...
        self.size = ldm.model_management.module_size(self.model)
        ...
        if patch_weights:
            ...
            for key in self.patches:
                ...
                if device_to is not None:
                    temp_weight = ldm.model_management.cast_to_device(
                ...
                if inplace_update:
                    ldm.utils.copy_to_param(self.model, key, out_weight)
                else:
                    ldm.utils.set_attr(self.model, key, out_weight)
                del temp_weight
            ...
            if patch_type == "diff":
                ...
                if alpha != 0.0:
                    if w1.shape != weight.shape:
                        ...
                        weight += alpha * ldm.model_management.cast_to_device(
                            w1, weight.device, weight.dtype
                        )
            elif patch_type == "lora":  # lora/locon
                mat1 = ldm.model_management.cast_to_device(
                    v[0], weight.device, torch.float32
                )
                mat2 = ldm.model_management.cast_to_device(
                ...
                if v[3] is not None:
                    ...
                    mat3 = ldm.model_management.cast_to_device(
                ...
                if w1 is None:
                    dim = w1_b.shape[0]
                    w1 = torch.mm(
                        ldm.model_management.cast_to_device(
                    ...
                    w1 = ldm.model_management.cast_to_device(
                ...
                if w2 is None:
                    ...
                    w2 = ldm.model_management.cast_to_device(
    ...
    def unpatch_model(self, device_to=None):
        ...
        if self.weight_inplace_update:
            for k in keys:
                ldm.utils.copy_to_param(self.model, k, self.backup[k])
        else:
            for k in keys:
                ldm.utils.set_attr(self.model, k, self.backup[k])
