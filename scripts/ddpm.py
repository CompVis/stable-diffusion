...
class UNet(DDPM):
    ...
    def p_sample_ddim(
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.0 or (index % min(total/10, 5) > 0 and (index <= total*(2/3) or total-index > 20)):
    ...
    def euler_sampling(
        ...
        for i in clbar(range(len(sigmas) - 1), name = "Samples", position = "first", prefixwidth = 12, suffixwidth = 28):
            ...
            if unconditional_conditioning is None or unconditional_guidance_scale == 1.0 or (i % min(len(sigmas)/10, 5) > 0 and (i >= len(sigmas)/3 or i > 20)):
                c_out, c_in = [
