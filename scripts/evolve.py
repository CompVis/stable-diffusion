...
def run_generation(precision_scope, opt, model, batch_size, data, sampler, start_code, wm_encoder, sample_path, base_count, n_rows, grid_count, outpath, prompt_delta=None):
    with torch.no_grad():
        with precision_scope("cuda"):
            with model.ema_scope():
                ...
                for n in trange(opt.n_iter, desc="Sampling"):
                    for idx, prompts in enumerate(tqdm(data, desc="data")):
                        ...
                        if prompt_delta is not None:
                            c = torch.add(c, prompt_delta[idx])
                ...
                if not opt.skip_grid:
                    ...
                    grid = torch.stack([all_samples[0]], 0) # only the first set
                    ...
                    os.system(f"cp {outpath}/grid-{grid_count:04}.png {outpath}/current.png")
                    grid_count += 1
...
def main():
    ...
    parser.add_argument(
        ...
        help="default mutation rate",
    )
    parser.add_argument(
        "--prompt_mutation_rate",
        type=float,
        default=DEFAULT_PROMPT_MUTATION_RATE,
        help="default prompt mutation rate",
    )
    parser.add_argument(
        "--from-file",
        type=str,
    ...
    while True:
        try:
            ...
            print(f"Using start code {response}")
            last_start_code = start_code
