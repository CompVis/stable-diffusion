...
from ldm.util import HWC3, instantiate_from_config, max_tile, resize_image, set_attr
...
def managePrompts(
    ...
    if translate == "true":
        ...
        if modelLM is not None:
            try:
                ...
                cardMemory = (
                    torch.cuda.get_device_properties("cuda").total_memory / 1073741824
                )
                usedMemory = cardMemory - (torch.cuda.mem_get_info()[0] / 1073741824)
    ...
    if modelType == "pixel" and promptTuning == "true":
        ...
        negativeList = [
            negative,
            "mutated, noise, frame, film reel, snowglobe, deformed, stock image, watermark, text, signature, username",
        ]

        if any(
            f"{_}.pxlm" in loraFiles
            for _ in [
                "topdown",
                "isometric",
                "neogeo",
...
def render(
    ...
    if pixelvae == "true":
        ...
        x_sample = modelPV.run_cluster(
            samples_ddim[i : i + 1],
            threshold=denoise,
            wrap_x=bool(tilingX == "true"),
            wrap_y=bool(tilingY == "true"),
        )
        ...
        x_sample = [
            modelFS.decode_first_stage(samples_ddim[i : i + 1].to(device))[0].cpu()
        ]
...
def txt2img(
    ...
    runs = (
        math.floor(n_iter / batch)
        if n_iter % batch == 0
        else math.floor(n_iter / batch) + 1
    )
    ...
    if W // 8 >= 96 and H // 8 >= 96 and upscale == "true":
        rprint(
            f"[#48a971]Pre-generating[white] composition image with [#48a971]{g_ddim_steps}[white] steps at [#48a971]{gWidth * 8}[white]x[#48a971]{gHeight * 8} ([#48a971]{(gWidth * 8) // pixelSize}[white]x[#48a971]{(gHeight * 8) // pixelSize}[white] pixels)"
        )
    ...
    for i, loraFile in enumerate(loraFiles):
        ...
        if loraFile != "none":
                ...
                with open(lora_filename, "rb") as enc_file:
                    ...
                    with open(lora_filename, "wb") as dec_file:
                        ...
                        try:
                            ...
                            loras.append(None)
                            rprint(
                                f"[#ab333d]Modifier {os.path.splitext(loraFile)[0]} could not be loaded, the file may be corrupted"
                            )
                            continue
            else:
                loras.append(load_lora(lora_filename, model))
            ...
            apply_lora()
            rprint(
                f"[#494b9b]Using [#48a971]{os.path.splitext(loraFile)[0]} [#494b9b]LoRA with [#48a971]{loraWeights[i]}% [#494b9b]strength"
            )
        else:
            loras.append(None)
    ...
    with torch.no_grad():
        ...
        conditioning = [[c[0], {"pooled_output": None}]]
        uncontrolled_conditioning = [[uc[0], {"pooled_output": None}]]

        # apply controlnet
        print("Applying controlnet...")
        (controlled_conditioning,) = apply_controlnet(
            conditioning, controlnet, image, control_strength
        )
        ...
        for n in clbar(
            ...
            with precision_scope("cuda"):
                ...
                rprint(f"samples_ddim: {samples_ddim.shape}")
                ...
                for i in range(batch):
                    ...
                    x_sample_image.save(os.path.join(outpath, file_name + ".png"))
...
def img2img(
    ...
    with torch.no_grad():
        ...
        with precision_scope("cuda"):
            ...
            init_latent_base = modelFS.get_first_stage_encoding(
            ...
            with precision_scope("cuda"):
                try:
                    ...
                    if pixelvae == "true":
                        ...
                        denoise = 0.08
                        x_sample = modelPV.run_cluster(
                            samples_ddim, threshold=denoise, wrap_x=False, wrap_y=False
                        )
                    else:
                        modelFS.to(device)
                        # Decode the samples using the first stage of the model
                        x_sample = [
                            modelFS.decode_first_stage(samples_ddim.to(device))[0].cpu()
                        ]
...
async def server(websocket):
    ...
    async for message in websocket:
        if re.search(r"txt2img.+", message):
            ...
            try:
                # Extract parameters from the message
                path, prompt, negative, n_iter, seed = searchString(
                    message, "dpath", "dprompt", "dnegative", "diter", "dseed", "end"
                )
            ...
            try:
                ...
                await websocket.send(
                    f"returning benchmark {max(256, maxSize-64)}"
                )  # We subtract 64 to leave a little VRAM headroom, so it doesn't OOM if you open a youtube tab T-T
            except Exception as e:
                rprint(f"\n[#ab333d]ERROR:\n{traceback.format_exc()}")
                play("error.wav")
                await websocket.send("returning error")
            ...
            try:
                # Extract parameters from the message
                modelPath, numFiles = searchString(
                    message, "dmodelpath", "dnumfiles", "end"
                )
